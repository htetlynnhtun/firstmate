...
# `agent get`. It answers for pi and agy, from two live signals:
...
#     runtime-backends.md "Agent liveness name sources") or agy, falling back to
#     tmux's own foreground-derived #{pane_current_command}. A pane whose
#     agent died to a shell has no verified foreground process and gets NO identity,
#     which is exactly what keeps the strict blank-row rule honest: a blank
#     row between two stale rules stays unknown.
#   - status: the harness's verified busy footer via fm_pane_busy_state, mapped onto the
#     idle/working vocabulary herdr's probe reports natively.
# Prints "<agent><TAB>idle" or "<agent><TAB>working"; exits 1 when the pane is not a
# live verified separated-composer agent.
...
EOF
  ...
  case "$found" in
    pi)
      ...
      ;;
    agy)
      status=$(fm_pane_busy_state "$target" agy)
      case "$status" in
        busy) printf 'agy\tworking' ;;
        idle) printf 'agy\tidle' ;;
        *) return 1 ;;
      esac
      ;;
    *) return 1 ;;
