...
if [ "$CMD_SET" -eq 0 ]; then
  ...
  CMD=$(printf '%s' "$PAYLOAD" | jq -r '(.toolInput.command // .tool_input.command // .toolInput.CommandLine // .tool_input.CommandLine // .args.CommandLine // .CommandLine // empty)' 2>/dev/null) || exit 0
  [ -n "$CMD" ] || exit 0
