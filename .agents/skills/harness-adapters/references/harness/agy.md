...
| Composer | Separated composer (`>` between two horizontal `─` rules); classified as `empty` when idle with verified agy identity, `pending` when containing input, and `unknown` without identity or during active turns. |
...
There is no launch flag that suppresses the dialog, but agy honours a `trustedWorkspaces` entry in the captain's own `~/.gemini/antigravity-cli/settings.json` written ahead of launch (verified live), so `../../../../../bin/fm-spawn.sh` pre-registers the worktree or secondmate home through `../../../../../bin/fm-agy-trust.sh` before launch, the claude shape: the helper refuses anything but a linked worktree or seeded secondmate home, records both the logical pane path and its resolved form because agy compares the logical cwd, and preserves every other key in the store.
...
agy is registered in `../../../../../bin/fm-session-lock-lib.sh` (`FM_HARNESS_RE` and `FM_HARNESS_NAMES`) so an agy primary or secondmate owns the session lock.
...
`../../../../../docs/supervision-protocols/agy.md` owns the agy foreground checkpoint protocol.
Tracked hook configuration in `.agents/hooks.json` wires `PreInvocation` (`bin/fm-sessionstart-agy.sh`), `PreToolUse` (`bin/fm-arm-pretool-check.sh`, `bin/fm-cd-pretool-check.sh`, and `bin/fm-subagent-pretool-check.sh`), and `Stop` (`bin/fm-turnend-guard-agy.sh`).
